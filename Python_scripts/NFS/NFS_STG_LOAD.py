...
def multi_processing_function(argv, file):
    ...
    NFS_table_mapping = json.loads(config_connection.execute('''select mapping_dict from table_mapping where 
                                                            process_name='{0}' 
    ...
    try:
        table_name = NFS_table_mapping[fname]
        if(fname=="NFS_ACTYD" or fname=="NFS_NAMADRD" ):
               index1 = 0
               index2 = 0
               ignore_index = 0
               ignore_top = 1
